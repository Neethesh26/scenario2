...
export default function Home() {
  ...
  const handleForm = async () => {  
    ...
    const { result, error } = await addData("dailyData", getAuth().currentUser.uid, data);
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      <div className="flex flex-col items-center justify-center">
        ...
        <form onSubmit={handleForm} class="max-w-sm mx-auto">
            ...
            <div>
                <label for="medTaken" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Taken Medication</label>
                <input type="text" id="medTaken" value={medTaken} onChange={(e) => setMedTaken(e.target.value)} class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
            </div>
